...
//            $media_id=$callback[1];
            ...
            $media_i=$media_json->content;
            $media_obj=json_decode($media_i);
...
//        echo $atoken;
        $postdata =array('name'=>'张','access_token'=>'vMg36dE2WO4pAqFLuUioNYF80fQFeyhUmYxbwzPcQF90mpW49nqKUcTjkQs3G0g3dzQ96vfPpeH4uiy6UrzlXHWhfCLUvhBliDC4GtcAGjQKIxr45-MSd2TgDo1Znu_ELBPgAHDWMA');
//        $media_id =$this->postTo($postdata);
        $url='http://zb.jentmc.cn/index.php?s=/addon/Toy/Toy6/checks_img';
        $media_id=$this->http($url,$postdata);
        echo 'media:'.$media_id[1];
...
//        print_r($s);
    ...
    function postTo($data){
        ...
        $json = $this->http($url,$data);
//        dump($json);
//        print_r('json:'.$json."\n");
        $media_id = $json[1];
//        $obj2=json_decode($obj);
//        dump($media_id);
//        print_r('id:'.$obj->media_id."\n");
