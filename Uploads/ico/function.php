...
function getWeixinUserInfo($openid, $token) {
	...
	$param2 ['access_token'] = 'gh_df04d7a5add0';
	$param2 ['openid'] = 'ojaTlvynCpx04Qr_Dd-QivKNt6y4';
	$param2 ['lang'] = 'zh_CN';
...
function get_access_token($token = '') {
	...
	$data = json_decode(file_get_contents($token."access_token.json"));
    if ($data->expire_time < time()) {
      ...
      if ($access_token) {
        ...
        $fp = fopen($token."access_token.json", "w");
